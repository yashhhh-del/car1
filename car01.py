...
if uploaded_file is not None:
    ...
    # Dataset Validation
    # -------------------------------
    required_column = 'Market_Price(INR)'
    if required_column not in df.columns:
        st.error(f"❌ Dataset must include '{required_column}' column.")
    ...
    X = df.drop(columns=[required_column], errors='ignore')
    y = df[required_column]
    ...
    with col1:
        fig, ax = plt.subplots()
        sns.histplot(df[required_column], kde=True, ax=ax)
    ...
    with col2:
        if 'Fuel_Type' in df.columns:
            fig, ax = plt.subplots()
            sns.boxplot(x='Fuel_Type', y=required_column, data=df, ax=ax)
